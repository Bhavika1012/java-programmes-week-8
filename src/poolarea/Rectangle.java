...
public class Rectangle {
    ...
    public Rectangle(double width, double length) { // Constructor with two parameters
        // Initialize width and length fields with params
        this.width = width;
        this.length = length;

        if (width < 0) { //Check if width is less than zero
            this.width = 0; //set width to zero
        }

        if (length < 0) { //Check if length is less than zero
            this.length = 0; //Set length to zero
        }
    }

    public double getWidth() { // Method to get the width
        return width;
    }

    public double getLength() { // Method to get the length
        return length;
    }

    public double getArea() { // Method to get the area of rectangle
        return width * length;
    }
}
